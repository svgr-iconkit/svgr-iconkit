...
module.exports = {
  ...
  exec: async (
    ...
    const fontFiles = FS.readdirSync(resolvedSourceDir).filter((file) => {
    ...
    console.log(commandName + ": total icons=%o", fontFiles.length);
    ...
    for (const fontFileName of fontFiles) {
      ...
      const sourcePath = Path.join(resolvedSourceDir, fontFileName);
